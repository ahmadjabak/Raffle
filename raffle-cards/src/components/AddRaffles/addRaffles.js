...
export default function AddRaffles() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (!name || !price || !image || !start || !end || !desc) {
      ...
      axios
        .post("http://localhost:5000/api/raffles", formData, {
          method: 'POST',
  ...
  const onSubmitHandlers = (e) => {
    ...
    if (window.confirm("Are you sure you want to add this raffle ?")) {
      axios
        .put(`http://localhost:5000/api/raffles/${location.state.id}`, formData, {
  ...
  const fetchRaffles = async () => {
    await fetch(`http://localhost:5000/api/get/${id || location.state.id}`, {
      method: 'GET',
      headers: {
